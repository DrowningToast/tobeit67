...
const FAQ = () => {
  return (
    <section className="bg-gradient-to-b from-[#333333] to-[#0C0C0C] py-16 flex flex-col gap-y-8 px-8 relative">
      <img
      ...
      <div className="flex flex-col text-xs">
        {pairs.map((pair, index) => {
          return (
            <div
              key={`faq-${index}`}
              className="text-center font-chonburi flex flex-col gap-y-1"
            >
              <h5 className="underline">Q</h5>
              <p>{pair.question}</p>
              <h5 className="underline">A</h5>
              <p>{pair.answer}</p>
            </div>
          );
        })}
